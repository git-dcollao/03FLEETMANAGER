        ...
        if vehiculo:
            if request.method == 'POST':
                ...
                if fecha_seleccionada == 'mostrarTodo':
                    ...
                    coordenadas_vehiculo = CoordenadasVehiculo.query.filter_by(id_vehiculo=id_vehiculo, fecha=fecha_seleccionada, estado = 1).order_by(CoordenadasVehiculo.fecha, CoordenadasVehiculo.hora).all()
            else:
...
def calcular_pendiente(coord_anterior, coord_actual):
    ...
    if delta_longitud == 0:
        pendiente = 0  # Representa una pendiente indefinida/infinita
    else:
