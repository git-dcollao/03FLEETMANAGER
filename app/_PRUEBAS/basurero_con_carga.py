...
def ver_trayecto(id_vehiculo):
    if 'username' in session:
        ...
        for i, coord in enumerate(coordenadas_vehiculo):
            ...
            # Encuentra la coordenada válida más cercana hacia atrás
            j = i - 1
            while j >= 0 and coordenadas_vehiculo[j].estado == 2:
                j -= 1

            if j >= 0:
                coord_anterior_valida = coordenadas_vehiculo[j]
            else:
                coord_anterior_valida = None

            if coord_anterior_valida:
                distancia = calcular_distancia(coord_anterior_valida, coord)
                tiempo_transcurrido = calcular_tiempo(coord_anterior_valida, coord)
                pendiente, delta_latitud, delta_longitud = calcular_pendiente(coord_anterior_valida, coord)
                ...
                coordenadas_json.append({
                    ...
                    'distancia': round(coord.distancia_m) if coord.distancia_m is not None else None,
                        ...
                        if vehiculo:
                            ...
                            if not existing_coordinate:
                                nueva_coordenada_vehiculo = CoordenadasVehiculo(
                                    latitud=utm_latitud,  # Almacenar la latitud convertida a UTM
                                    longitud=utm_longitud,  # Almacenar la longitud convertida a UTM
                                    grados_latitud=row['Latitud'],  # Mantener la latitud original en grados
                                    grados_longitud=row['Longitud'],  # Mantener la longitud original en grados
                                    fecha=fecha_formateada,
                                    hora=row['Hora'],
                                    vehiculo=vehiculo,
                                    estado=1,
                    ...
                    for i in range(len(coordenadas_vehiculo)):
                        ...
                        if i == 0 or (i > 0 and coordenadas_vehiculo[i - 1].fecha != coord_curr.fecha):
                            ...
                            if response.status_code == 200:
                                data = response.json()
                                distancia_r = data['routes'][0].get('distance', 0)
                            else:
                                distancia_r = 0
                        ...
                        if int(coord_curr.distancia_r) > int(coord_curr.distancia_m) + 2:
                            coord_curr.estado = 2
