...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root@localhost/fleet_manager'
...
def ver_trayecto(id_vehiculo):
    if 'username' in session:
        vehiculo = Vehiculo.query.get(id_vehiculo)
        coordenadas_vehiculo = CoordenadasVehiculo.query.filter_by(id_vehiculo=id_vehiculo).order_by(CoordenadasVehiculo.fecha.desc(), CoordenadasVehiculo.hora).all()
        ...
        coordenada_anterior = None  # Inicializar la coordenada anterior a None
        delta_latitud_anterior = 0
        delta_longitud_anterior = 0
        pendiente_anterior = 0
        for i, coord in enumerate(coordenadas_vehiculo):
            if i > 0:
                distancia, tiempo_transcurrido = calcular_distancia_tiempo(coordenada_anterior, coord, tiempo_anterior)
                pendiente, delta_latitud, delta_longitud = calcular_pendiente(coordenada_anterior, coord)

                if not distancia:
                    continue

                if not delta_latitud or delta_longitud:
                    pass
                ...
                if (((caso_actual_1 and caso_antiguo_2) == True) or ((caso_actual_2 and caso_antiguo_1) == True) or ((caso_actual_3 and caso_antiguo_4) == True) or ((caso_actual_4 and caso_antiguo_3) == True)):
                    coord.estado = 2 # Coordenada no válida
                    db.session.commit()
                    continue
                ...
                coordenadas_json.append({
                    ...
                    'fecha': coord.fecha.strftime('%Y-%m-%d'),
                    'hora': coord.hora.strftime('%H:%M:%S'),
                ...
                tiempo_anterior = datetime.strptime(f"{coord.fecha} {coord.hora}", "%Y-%m-%d %H:%M:%S")

            coordenada_anterior = coord
            delta_latitud_anterior = delta_latitud
            delta_longitud_anterior = delta_longitud
            pendiente_anterior = pendiente
...
def calcular_pendiente(coord_anterior, coord_actual):
    delta_latitud = coord_actual.latitud - coord_anterior.latitud
    delta_longitud = coord_actual.longitud - coord_anterior.longitud
    
    if delta_longitud == 0:
        return None, None, None
    
    pendiente = delta_longitud / delta_latitud
...
def calcular_distancia_total(coordenadas_vehiculo):
    distancia_total = 0
    for i in range(1, len(coordenadas_vehiculo)):
        distancia, _ = calcular_distancia_tiempo(coordenadas_vehiculo[i-1], coordenadas_vehiculo[i])
        distancia_total += distancia
    return distancia_total

def calcular_tiempo_detenido(coordenadas_vehiculo):
    tiempo_detenido = 0
    for i in range(1, len(coordenadas_vehiculo)):
        distancia, tiempo_transcurrido = calcular_distancia_tiempo(coordenadas_vehiculo[i-1], coordenadas_vehiculo[i])
        velocidad = 0 if tiempo_transcurrido == 0 else distancia / tiempo_transcurrido
        if velocidad <= 0:
            tiempo_detenido += tiempo_transcurrido
    return tiempo_detenido

def calcular_tiempo_activo(coordenadas_vehiculo):
    tiempo_activo = 0
    for i in range(1, len(coordenadas_vehiculo)):
        distancia, tiempo_transcurrido = calcular_distancia_tiempo(coordenadas_vehiculo[i-1], coordenadas_vehiculo[i])
        velocidad = 0 if tiempo_transcurrido == 0 else distancia / tiempo_transcurrido
                ...
                try:
                    for index, row in df.iterrows():
                        ...
                        if vehiculo:
                            ...
                            # Verificar si la coordenada ya existe para el vehículo
